...
```{r}
tibble::tibble(
  nome = CursoRopen:::list_github_files(params$turma, "exercicios/"),
  link = paste0("https://curso-r.github.io/", params$turma, "/", nome),
  exercicio = glue::glue("[{nome}]({link})")
) %>%
    dplyr::filter(!stringr::str_detect(nome, "resolucao_")) %>% 
    dplyr::filter(stringr::str_detect(nome, "01-|02-|03-")) |> 
